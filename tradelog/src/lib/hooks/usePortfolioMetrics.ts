...
import { Metrics, Holding } from '../types';

export function usePortfolioMetrics(holdings: Holding[]) {
  const metrics: Metrics = useMemo(() => {
    if (!holdings || holdings.length === 0) {
      return { walletValue: 0, allTimePnl: 0, dailyPnl: 0, dailyPnlPercentage: 0 };
    }

    const totalWalletValue = holdings.reduce((acc, holding) => acc + (holding.currentValueUSD || 0), 0);
    const historicalWalletValue = holdings.reduce((acc, holding) => acc + (holding.historicalValueUSD || 0), 0);
