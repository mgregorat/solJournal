...
class BridgeTransport extends abstract_1.AbstractTransport {
    ...
    call({ session, name, data, protocol: customProtocol, thpState, signal, timeout, }) {
        return this.scheduleAction(async (signal) => {
            ...
            return (0, receive_1.receiveAndParse)(this.messages, () => Promise.resolve(this.success(Buffer.from(response.payload.data, 'hex'))), protocol, thpState);
        }, { signal, timeout });
    }
    send({ session, name, data, protocol: customProtocol, thpState, signal, }) {
        return this.scheduleAction(async (signal) => {
            const protocol = this.getProtocol(customProtocol);
            const bytes = (0, send_1.buildMessage)({
                messages: this.messages,
    ...
    receive({ session, protocol: customProtocol, thpState, signal, }) {
        return this.scheduleAction(async (signal) => {
            const protocol = this.getProtocol(customProtocol);
